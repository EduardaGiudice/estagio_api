...
public class LoginController {
    ...
    public ResponseEntity<?> autenticar(@RequestBody Login login) {
        try {
            ...
            Logins loginEntry = new Logins();
            loginEntry.setUsuario(usuarioAutenticado);
            loginEntry.setCreate_at(LocalDateTime.now());
            loginsRepo.save(loginEntry);
            ...
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(new ErrorResponse("Senha inválida."));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ErrorResponse("Erro inesperado. Tente novamente mais tarde."));
        }
    }
